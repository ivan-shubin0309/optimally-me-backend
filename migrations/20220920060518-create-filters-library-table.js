...
module.exports = {
  up(queryInterface) {
    const createFiltersLibraryTableSql = `
      CREATE TABLE IF NOT EXISTS filtersLibrary (
        ...
        FOREIGN KEY libraryRule (ruleId) REFERENCES rulesLibrary (id) ON DELETE CASCADE ON UPDATE CASCADE,
        PRIMARY KEY (id)
      ) ENGINE=INNODB CHARACTER SET=UTF8MB4 COLLATE UTF8MB4_UNICODE_CI;`.replace(/\s+/ig, ' ').trim();

    return queryInterface.sequelize.query(createFiltersLibraryTableSql);
  },
  down(queryInterface) {
    return queryInterface.sequelize.query('DROP TABLE IF EXISTS filtersLibrary;');
  }
};
