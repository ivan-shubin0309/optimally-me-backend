...
@Injectable()
export class DnaAgeBiomarkersFactory extends BiomarkersFactory {
    constructor(
    ...
    async createBiomarker(body: CreateDnaAgeBiomarkerDto, transaction?: Transaction): Promise<Biomarker> {
        let templateId;
        if (body.ruleName) {
        ...
        return this.create(Object.assign({ type: BiomarkerTypes.dnaAge, templateId }, body), transaction);
    }

    async createRule(body: CreateDnaAgeBiomarkerDto, transaction?: Transaction): Promise<Biomarker> {
        const ruleToCreate = Object.assign(
            {},
            body,
            { type: BiomarkerTypes.dnaAgeRule, name: body.ruleName, label: null, shortName: null }
        );
