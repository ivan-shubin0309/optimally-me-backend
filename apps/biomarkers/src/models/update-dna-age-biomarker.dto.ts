...
import { UpdateDnaAgeFilterDto } from './filters/update-dna-age-filter.dto';

export class UpdateDnaAgeBiomarkerDto implements ICreateBiomarker {
    ...
    @ApiProperty({ type: () => [UpdateDnaAgeFilterDto], required: true })
    @ArrayNotEmpty()
    @ArrayMaxSize(skinBiomarkerValidationRules.filtersMaxCount)
    @ValidateNested()
    @Type(() => UpdateDnaAgeFilterDto)
    readonly filters: UpdateDnaAgeFilterDto[];
}