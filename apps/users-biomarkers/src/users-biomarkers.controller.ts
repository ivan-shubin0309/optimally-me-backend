...
export class UsersBiomarkersController {
  ...
  async getBiomarkersList(@Query() query: GetUserBiomarkersListDto, @Request() req: Request & { user: SessionDataDto }): Promise<UserBiomarkersDto> {
    ...
    if (count) {
      ...
      rangeCounters = await this.usersBiomarkersService.getBiomarkerRangeCounters(lastResultIds, scopes, query.biomarkerType);
      ...
      await this.usersBiomarkersService.includeUserBiomarkerCounters(
        biomarkersList,
        req.user.userId,
        { afterDate: query.afterDate, beforeDate: query.beforeDate, biomarkerTypes: query.biomarkerType }
      );
    }
